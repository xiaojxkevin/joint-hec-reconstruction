...
class HandEyeBundleAdjustment:
    ...
    def jacobian_sparsity(self):
        ...
        for idx in range(self.n_views):
            ...
            for local_idx, point_idx in enumerate(pts3d_indices):
                for r in range(
                    current_residual + local_idx * 2,
                    current_residual + local_idx * 2 + 2,
                ):
    ...
    def objective_function(self, params):
        ...
        for idx in range(self.n_views):
            ...
            pts2d_projected = np.array(
                [
                    self.fx * pts3d_eye_normalized[:, 0] + self.cx,
                    self.fy * pts3d_eye_normalized[:, 1] + self.cy,
                ]
            ).T
